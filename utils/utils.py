...
def compute_fo_exchange(distance_matrix, routes, i, m, j, n, fo_prec):
    ...
    node (n-th of j-th route), so we subtract the cost associated with the previous and
    the next link of each node (e.g. remove the cost from m-1 to m and the one
    ...
    now we have a situation like this, imagine that m and n are free nodes:
    ...
    now we the value of the objective function like if the nodes are like this
    (look carefully how m and n are switched):

    route i: D -> ... ->  (m-1) -> (n) -> (m+1) -> ... -> D
    route j: D -> ... ->  (n-1) -> (m) -> (n+1) -> ... -> D

    remember: the exchange is "never" happened, because of this formula: we can
    avoid to recompute all the objective function, where the variation is only
    local in the exchange.

    _____

    This method is valid only if the routes are different. If the routes are
    ...
    if we switch m and n, they are like this:
    ...
    :param fo_prec: the value of the objective function before the exchange
    ...
    if i == j:
        ...
        if n == m:
            ...
            if n == m + 1:  # n follows m in the same route
...
def minimize_fo(instance):
    ...
    main cycle:
        For each node in each route, we check if exchanging it with one another node the
        objective function improves. If so, we save the coordinates of the exchange
        (i.e. indices of routes and nodes in the routes) and the value of the objective
        function if it's less than the previous maximum. So after we found the best exchange
        for a node, we do it and we can go on with the next node. Of course the
        exchanges must respect the constraints involved.

    Repeat this "main cycle" while the objective function is improving, that is its
    value is descending.

    So, an exchange is possible if:
    ...
    4) the first node is the first backhaul of the first route and
        the second node is the flast linehaul of the second route.
    ...
    fo_curr = objective_function(instance.distance_matrix, instance.curr_routes)
            ...
            for m in range(1, len(routes[i]) - 1):
                ...
                for j in range(len(routes)):
                    for n in range(1, len(routes[j]) - 1):
                        ...
                        fo_exchange = compute_fo_exchange(
                            ...
                            if not (i == j and m == n):
                                ...
                                if type_node_m == LINEHAUL_TYPE and type_node_n == LINEHAUL_TYPE:
                                    ...
                                    if (instance.curr_routes[i].linehauls_load() -
                                         routes[i][m].load + routes[j][n].load <=
                                         instance.vehicle_load) and \
                                        (instance.curr_routes[j].linehauls_load() -
                                         routes[j][n].load + routes[i][m].load <=
                                ...
                                # L <-> B
                                    ...
                                    # we check that the linehauls list of the first route has at least two
                                    # elemets and that the linehaul node is followed by a backhaul or
                                    # by a depot and that the backhaul node is preceded by a linehaul;
                                    ...
                                    if i != j and len(instance.curr_routes[i].linehauls) >= 2 and \
                                        ...
                                        if (instance.curr_routes[j].linehauls_load() +
                                            ...
                                            exchange_indices = [i, m - 1, j,
                                                                n - (len(instance.curr_routes[j].linehauls) + 1)]
                                ...
                                # B <-> L
                                    ...
                                    # we check that the linehauls list of the second route has at least two
                                    # elements and that the linehaul node is followed by a backhaul or
                                    # by a depot and that the backhaul node is preceded by a linehaul;
                                    # also they must be in different routes
                ...
                final_exchange(instance.curr_routes, exchange_indices, exchange_type)
